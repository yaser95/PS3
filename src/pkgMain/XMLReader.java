...
public class XMLReader {
    ...
    public static Book getBook(Catalog cat, String id){
        ArrayList<Book> books = cat.getBooks();
        for (Book book : books) {
            if(id.equals(book.getId()))
                return book;
        }
        throw new BookException("Error: Book with id " + id + " not found.");
    }
    
    public static void addBook(Catalog cat, Book newBook){
        ArrayList<Book> books = cat.getBooks();
        for (Book book : books) {
            if(newBook.getId().equals(book.getId()))
                 throw new BookException("Error: Book with id " + newBook.getId() + " already exist.");
        }
        books.add(newBook);
    }
